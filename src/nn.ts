...
export class Tokenizer {
  ...
  constructor(modelName: string, max_len: number) {
    ...
    this.json_url = chrome.runtime.getURL(
      "res/models/" + modelName + "/tokenizer.json"
    );
  }
  ...
  private cleanStr(inputIntent: string): string {
    // strip punctuation
    const noPunc: string = inputIntent.replace(
      /[.,\/#!$%\^&\*;:{}=\-_`~()]/g,
      ""
    );
...
export class IntentClassifier {
  ...
  async predict(intent: string, threshold: number = 0.5): Promise<boolean> {
    ...
    // predict
    const predictionTensor: tf.Tensor = this.model.predict(
      inputTensor
    ) as tf.Tensor;
  ...
  async loadModel(modelName: string) {
    ...
    const modelDir = chrome.runtime.getURL(
      "res/models/" + modelName + "/model.json"
    );
