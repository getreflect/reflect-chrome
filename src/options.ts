...
function restoreSavedOptions(): void {
  chrome.storage.sync.get(null, (storage) => {
    (document.getElementById("whitelistTime") as HTMLFormElement).value =
      storage.whitelistTime;
...
function updateButtonListeners(): void {
  ...
  for (const button of <any>buttons) {
    button.addEventListener("click", () => {
      ...
      // get url
      const url: string = document.getElementById(button.id[0] + "site")
        ?.innerHTML;
...
function drawIntentListTable(): void {
  ...
  chrome.storage.sync.get(null, (storage) => {
    ...
    const previousIntents: HTMLElement = document.getElementById(
      "previousIntents"
    );
...
function addUrlToFilterList(): void {
  // get urlInput
  const urlInput: HTMLFormElement = document.getElementById(
    "urlInput"
  ) as HTMLFormElement;
