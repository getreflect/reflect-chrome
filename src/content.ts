...
function getStrippedUrl(): string {
  // match url
  const activeURL: RegExpMatchArray | null = window.location.href.match(
    /^[\w]+:\/{2}([\w\.:-]+)/
  );
  if (activeURL != null) {
...
function loadBlockPage(strippedURL: string): void {
  ...
  $.get(chrome.runtime.getURL("res/pages/prompt.html"), (page) => {
    ...
    $("#linkToOptions").attr(
      "href",
      chrome.runtime.getURL("res/pages/options.html")
    );
    ...
    $("#top-left-blob").attr(
      "src",
      chrome.runtime.getURL("res/blob-big-2.svg")
    );
    $("#bottom-right-blob").attr(
      "src",
      chrome.runtime.getURL("res/blob-big-1.svg")
    );
